...
from groove.retrieval.embed import embed_segments, load_encoder, pool_to_tracks
...
def main():
    ...
    parser.add_argument("--k", type=int, default=10, help="k for recall/purity.")
    ...
    seg_df, seg_emb = embed_segments(manifest, paths.segments_dir, model, mel, cfg, device, args.batch_size)
    ...
    print("  recall@k (same-track): " + "  ".join(f"@{k}={v:.3f}" for k, v in recall.items()))
    if genres is not None:
        purity, base = genre_purity(track_emb, genres, k=args.k)
        print(f"  genre purity@{args.k}:    {purity:.3f}  (random baseline {base:.3f})")
    else:
        print("  genre purity:          skipped (no genre in catalog)")
    print(f"  bpm coherence:         nn={nn_bpm:.1f}  random={rand_bpm:.1f}  ratio={bpm_ratio:.3f}")
