...
Usage:
    python scripts/train.py --name stats-pool
    python scripts/train.py --debug                 # quick run on Mac (MPS)/CPU
    python scripts/train.py --pooling gem --batch_size 768
"""
import argparse
...
def main():
    ...
    parser.add_argument("--name", default=None, help="Run name (folder suffix).")
    ...
    parser.add_argument("--pooling", choices=["stats", "attention", "gem"])
    ...
    train(cfg, name=args.name, debug=args.debug)
